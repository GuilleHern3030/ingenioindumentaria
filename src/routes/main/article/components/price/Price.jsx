...
export default ({price, discount=0, installments=0, installmentsPrice=0, fees=0, t}) => {
    ...
    return dataLoaded && <div className={styles.container}>

        {/* Descuento */}
        { (discount > 0) && 
            <div className={styles.div}>
                <p className={styles.strike}><strike>{`${coinSymbol} ${price}`}</strike></p>
                <p className={styles.discount}><b>{`-${discount}%`}</b></p>
            </div>
        }

        {/* Precio */}
        <p className={styles.price}>{`${coinSymbol} ${discount > 0 ? (price - price * discount / 100).toFixed(decimals) : price}`}</p>

        { (installments > 0) && // Cuotas
            <div className={styles.div}>
                <p className={styles.installments}>{`${installments} ${t('installments-free-interest')} `}</p>
                <p className={styles.installments_price}><b>{`${coinSymbol}${installmentsPrice}*`}</b></p>
            </div>
        }

        { (fees > 0) && // Precio sin impuestos
            <p className={styles.price_no_fees}>{`${t('price_no_iva')} ${coinSymbol}${(price - price * fees / 100).toFixed(decimals)}`}</p> 
        }
